module "gcp_gke_stage_workspace" {
  source       = "./workspace"
...
resource "tfe_notification_configuration" "gcp_gke_stage_notification" {
  ...
  url                   = var.webhook_url
  workspace_external_id = module.gcp_gke_stage_workspace.external_id
}

# GCP credentials
resource "tfe_variable" "gcp_gke_stage_credentials_variable" {
  key          = "gcp_credentials"
